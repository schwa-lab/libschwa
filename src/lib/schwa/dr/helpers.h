...
namespace schwa {
  namespace dr {
    ...
    /**
     * Base template definition used to make \ref REVERSE_SLICES work. The types used in the
     * template arguments here are what are provided by decltype in \ref REVERSE_SLICES, but they
    ...
    /**
     * Template specialisation to make \ref REVERSE_SLICES work in the mutually exclusive case. That
    ...
    /**
     * Template specialisation to make \ref REVERSE_SLICES work in the non-mutually exclusive case.
    ...
    /**
     * @def REVERSE_SLICES(source_store, target_store, slice_attr, pointer_attr)
     *
     * A macro that wraps the type inference used in the template magic of ReverseSlices.
     * \code{.cpp}
     * REVERSE_SLICES(&Doc::sents, &Doc::tokens, &Sent::span, &Token::sent)(doc);
     * \endcode
     *
     ...
     * \code{.cpp}
     * REVERSE_SLICES(&Doc::sents, &Doc::tokens, &Sent::span, &Token::sents)(doc);
     * \endcode
     *
     * The functor returned by this macro can be stored and reused instead of just created and used
     * once.
     * \code{.cpp}
     * auto x = REVERSE_SLICES(&Doc::sents, &Doc::tokens, &Sent::span, &Token::sent);
     * ...
     * for (Doc &doc : docs)
     *   x(doc);
     * \endcode
     **/
    #define REVERSE_SLICES(source_store, target_store, slice_attr, pointer_attr) schwa::dr::ReverseSlices<decltype(source_store), source_store, decltype(target_store), target_store, decltype(slice_attr), slice_attr, decltype(pointer_attr), pointer_attr>()
