...
#include <cstdio>       // fprintf, fopen, fread, fclose
...
using namespace schwa::third_party::xxhash;
...
// MACRO
//**************************************
#define DISPLAY(...) std::fprintf(stderr, __VA_ARGS__)
...
static size_t BMK_findMaxMem(U64 requestedMem)
{
    ...
    while (!testmem)
    {
        ...
        testmem = (BYTE*) std::malloc((size_t)allocatedMemory);
    }
    std::free (testmem);
...
int BMK_benchFile(char** fileNamesTable, int nbFiles, int selection)
{
    ...
    while (fileIdx<nbFiles)
    {
        ...
        inFile = std::fopen( inFileName, "rb" );
        if (inFile==NULL)
        {
        ...
        buffer = (char*)std::malloc((size_t )benchedSize+16);
        if(!buffer)
        {
        ...
        // Fill input buffer
        DISPLAY("\rLoading %s...        \n", inFileName);
        readSize = std::fread(alignedBuffer, 1, benchedSize, inFile);
        std::fclose(inFile);
...
int main(int argc, char** argv)
{
    ...
    if (argc<1) { badusage(argv[0]); return 1; }
    ...
    if(!input_filename) { filenamesStart=0; }
