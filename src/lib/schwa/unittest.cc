...
public:
  BytesPrinter(const uint8_t *expected, size_t expected_size, const uint8_t *actual, size_t actual_size) :
      _expected(expected),
...
inline std::ostream &
operator <<(std::ostream &out, const BytesPrinter &obj) {
  return obj(out);
}


std::ostream &
BytesPrinter::operator ()(std::ostream &out) const {
  size_t a = 0, e = 0;
...
compare_bytes(const uint8_t *actual, const size_t actual_size, const uint8_t *expected, const size_t expected_size, ut::TestResults &results, const ut::TestDetails &details) {
  if (actual_size != expected_size) {
    ...
    msg << BytesPrinter(expected, expected_size, actual, actual_size);
