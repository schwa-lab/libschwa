...
namespace schwa {
  namespace config {
    ...
    public:
      Cfg(const std::string &name, const std::string &desc, Flags flags=SHOW_ALL, unsigned short order=0);
      Cfg(Cfg &cfg, const std::string &name, const std::string &desc, Flags flags=SHOW_ALL, unsigned short order=0);
      virtual ~Cfg(void) { }
      ...
      virtual void
      writeln_help(std::ostream &out, std::string prefix="", bool full=false) const {
        out << DESC << " options:\n";
        write_help(out, prefix, full);
      }
      virtual void
      writeln_config(std::ostream &out, std::string prefix="", bool root=true) const {
        write_config(out, prefix, root);
      }
      virtual void
      writeln_preface(std::ostream &out, std::string prefix="", bool root=true) const {
    ...
    public:
      Main(const std::string &name) : Cfg("", "main program", SHOW_ALL), _option_version(*this), _option_licence(*this) {
        PROGRAM_NAME = name;
      }
      virtual ~Main(void) { }
    ...
    public:
      ...
      Directory(const std::string &name, const std::string &desc, const OpPath *base=0, Flags flags=SHOW_ALL);
      Directory(Cfg &cfg, const std::string &name, const std::string &desc, const OpPath *base=0, Flags flags=SHOW_ALL);
